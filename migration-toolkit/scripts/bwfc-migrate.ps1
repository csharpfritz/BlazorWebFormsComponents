...
function Convert-EventHandlerWiring {
    <#
    .SYNOPSIS
        Adds @ prefix to event handler attribute values in markup (GAP-32).
    .DESCRIPTION
        Transforms OnClick="Handler" → OnClick="@Handler" in .razor markup.
        Only targets On[A-Z]* attributes with bare C# identifier values (method names).
        Values already starting with @ or containing non-identifier characters are skipped.
    #>
    param(
        [string]$Content,
        [string]$RelPath
    ...
    # Match On[UpperCase]Word="BareIdentifier" — event handler attributes with method name values
    $eventAttrRegex = [regex]'(On[A-Z]\w+)="([A-Za-z_]\w*)"'
    $matches = $eventAttrRegex.Matches($Content)

    if ($matches.Count -gt 0) {
        $Content = $eventAttrRegex.Replace($Content, '$1="@$2"')
        Write-TransformLog -File $RelPath -Transform 'EventWiring' -Detail "Added @ prefix to $($matches.Count) event handler attribute(s)"
    }

    return $Content
}

function Get-DataBindMap {
    <#
    .SYNOPSIS
        Pre-scans code-behind for DataSource assignment patterns (GAP-09).
    .DESCRIPTION
        Returns a hashtable mapping control IDs to generated backing field names.
        Used for cross-file correlation between code-behind DataBind patterns and markup Items attributes.
    #>
    param([string]$CodeBehindContent)

    $map = @{}
    $dsRegex = [regex]'(\w+)\.DataSource\s*='
    $dsMatches = $dsRegex.Matches($CodeBehindContent)

    foreach ($m in $dsMatches) {
        $controlName = $m.Groups[1].Value
        if ($controlName -in @('this', 'base', 'var', 'return', 'new')) { continue }
        $fieldName = '_' + $controlName.Substring(0,1).ToLower() + $controlName.Substring(1) + 'Data'
        $map[$controlName] = $fieldName
    }

    return $map
}

function Convert-DataBindPattern {
    <#
    .SYNOPSIS
        Transforms DataSource/DataBind() patterns in code-behind (GAP-09).
    .DESCRIPTION
        Detects controlName.DataSource = expression; and controlName.DataBind();
        Converts DataSource assignment to backing field assignment.
        Removes DataBind() calls.
        Generates private field declarations in the class body.
    #>
    param(
        [string]$Content,
        [string]$RelPath
    ...
    $dataBindMap = Get-DataBindMap -CodeBehindContent $Content
    if ($dataBindMap.Count -eq 0) { return $Content }

    $transformCount = 0

    # Step 1: Replace DataSource assignments with field assignments
    foreach ($controlName in @($dataBindMap.Keys)) {
        $fieldName = $dataBindMap[$controlName]
        $escaped = [regex]::Escape($controlName)
        $assignRegex = [regex]"${escaped}\.DataSource\s*=\s*(.+?);"
        $assignMatches = $assignRegex.Matches($Content)
        if ($assignMatches.Count -gt 0) {
            $Content = $assignRegex.Replace($Content, "$fieldName = `$1;")
            $transformCount += $assignMatches.Count
        }
    }

    # Step 2: Remove DataBind() calls for mapped controls
    foreach ($controlName in @($dataBindMap.Keys)) {
        $escaped = [regex]::Escape($controlName)
        $dbRegex = [regex]"(?m)^\s*${escaped}\.DataBind\(\);\s*\r?\n?"
        $Content = $dbRegex.Replace($Content, '')
    }

    # Step 3: Remove any remaining orphan DataBind() calls
    $genericDbRegex = [regex]'(?m)^\s*\w+\.DataBind\(\);\s*\r?\n?'
    $genericDbMatches = $genericDbRegex.Matches($Content)
    if ($genericDbMatches.Count -gt 0) {
        foreach ($m in $genericDbMatches) {
            Write-TransformLog -File $RelPath -Transform 'DataBind' -Detail "Removed orphan DataBind() call"
        }
        $Content = $genericDbRegex.Replace($Content, '')
    }

    # Step 4: Generate field declarations after class opening brace
    $fieldBlock = "`n    // Data binding fields (generated by L1 migration)`n"
    foreach ($entry in $dataBindMap.GetEnumerator()) {
        $fieldBlock += "    private IEnumerable<object> $($entry.Value);`n"
    }

    $classRegex = [regex]'(public\s+partial\s+class\s+\w+[^{]*\{)'
    if ($classRegex.IsMatch($Content)) {
        $Content = $classRegex.Replace($Content, "`$1`n$fieldBlock", 1)
    }

    Write-TransformLog -File $RelPath -Transform 'DataBind' -Detail "Converted $transformCount DataSource/DataBind pattern(s) for: $($dataBindMap.Keys -join ', ')"

    return $Content
}

function Add-DataBindItemsAttribute {
    <#
    .SYNOPSIS
        Injects Items="@_fieldName" on markup tags correlated with DataBind map (GAP-09).
    .DESCRIPTION
        Uses the control-ID-to-field-name map from Get-DataBindMap to add Items attributes
        to matching component tags in the markup.
    #>
    param(
        [string]$Content,
        [hashtable]$DataBindMap,
        [string]$RelPath
    )

    if (-not $DataBindMap -or $DataBindMap.Count -eq 0) { return $Content }
    ...
    foreach ($entry in $DataBindMap.GetEnumerator()) {
        ...
        # Insert Items attribute after the matching id attribute
        $idRegex = [regex]"(id=`"$controlId`")"
        if ($idRegex.IsMatch($Content)) {
            $Content = $idRegex.Replace($Content, "`$1 Items=`"@$fieldName`"")
            $addedCount++
        }
    }

    if ($addedCount -gt 0) {
        Write-TransformLog -File $RelPath -Transform 'DataBind' -Detail "Added Items attribute to $addedCount control(s) in markup"
    }
...
"@
        ...
        # --- GAP-09: DataSource/DataBind pattern conversion ---
        $annotatedContent = Convert-DataBindPattern -Content $annotatedContent -RelPath $RelPath
...
function Convert-WebFormsFile {
    ...
    # --- GAP-09: Pre-scan code-behind for DataBind patterns (cross-file correlation) ---
    $dataBindMap = @{}
    if ($extension -in @('.aspx', '.ascx')) {
        $cbPreScanPath = $SourceFile + '.cs'
        if (Test-Path $cbPreScanPath) {
            $cbPreScanContent = Get-Content -Path $cbPreScanPath -Raw -Encoding UTF8
            $dataBindMap = Get-DataBindMap -CodeBehindContent $cbPreScanContent
        }
    }
    ...
    $content = Convert-EventHandlerWiring -Content $content -RelPath $relativePath
    ...
    # --- GAP-09: Inject Items attributes from DataBind map ---
    $content = Add-DataBindItemsAttribute -Content $content -DataBindMap $dataBindMap -RelPath $relativePath

    # Clean up leftover blank linesfrom removed directives (collapse 3+ consecutive blank lines to 2)
